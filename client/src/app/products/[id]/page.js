        ...
        <div className="md:flex md:w-2/3 gap-6">
          ...
          <div className="flex md:flex-col gap-4 md:overflow-y-auto md:max-h-[500px]">
            {product.images.map((img, index) => (
              <Image
                key={index}
                src={img.startsWith('http') ? img : `${process.env.NEXT_PUBLIC_API_BASE}/${img}`}
          ...
          <div className="flex-1 mt-6 md:mt-0 relative overflow-hidden rounded-lg shadow-md">
            <Image
              src={product.images[selectedImage].startsWith('http') ? product.images[selectedImage] : `${process.env.NEXT_PUBLIC_API_BASE}/${product.images[selectedImage]}`}
